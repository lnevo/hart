...
- JMRI **Dispatcher System** (`jython/DispatcherSystem`, [help](https://www.jmri.org/help/en/html/scripthelp/DispatcherSystem/DispatcherSystem.shtml)) Stage 1+2 run on the Pi against `tables.xml` (replaces last session's hand-built transit). Stations = blocks with `stop` in the comment: **West Main Ext, Main West, Main East, McKees Rocks, McKeesport** — this covers the full mainline circuit, so a complete loop can be chained (WME → Main West → Main East → McKeesport → McKees Rocks → WME). Generated: station buttons/icons on My Layout, a "Dispatcher System" command panel, 41 sections, 40 transits, 80 traininfo files (fwd+rvs per pair), and a `Run Dispatcher` Logix (`IX:DSLX:1`) that starts the run threads from the panel button.
