...
**Path in hart:** `external/sts-docker`
...
STS PHP runtime (Docker). Pipeline 14 consumer. Desktop `Car Cards/sts-docker/` is a bench copy; canonical path is `~/sts/sts-docker` or `external/sts-docker` after submodule init.

## Re-pin

1. Update commit in this file and `cross-repo/SUBMODULE_MANIFEST.yaml`.
2. `cd external/sts-docker && git fetch && git checkout <pin>`.
