...
3. `jmri/scripts/mqtt_signalhead_publisher.py` — Digicon SML MQTT bridge: main-window **SML Enabled / SML Disabled** toggle; when Enabled, publish IH appearances on `track/signalhead/<packed>`; when Disabled (global or per-mast SML off), apply `track/signalmast/<packed>` → IH and (on per-mast disable edges / operator Disable) publish `Unheld`. Answers `track/bridge/sml_mode` **query** with **enabled** while globally Enabled. **Boot:** holds Digicon SML off, **reads** `sml_mode` only (no retain publish); missing/`disabled` → take Digicon (enable); `enabled` → stay Disabled with **no Unheld**.
