...
Digicon SML MQTT controller — toggle **SML Enabled / SML Disabled**, SET publish when Enabled, mast→IH when SML off, per-mast `Unheld` on disable **edges**, `track/bridge/sml_mode` query ACK. Digicon SML pairs are stored **Enabled=no** in tables. Stored-Enabled boot publishes **`enabled_on_boot`**; agents abort (immediate uncheck, no RELEASE) then **`aborted`**; Digicon resumes **`enabled`** after ~3s. Clean boot holds dests off, **reads** `sml_mode` (no retain publish); missing/`disabled` → enable Digicon; `enabled` / boot-abort tokens → stay Disabled with no Unheld. SML / SHSM still own aspects while Enabled; Held remains CATS/USS veto.
