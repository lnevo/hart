...
Princess east exits are **2-head** (main vs OS K-1/OS K-2 restricting). Balloon intermediates **Mast 2035** / **Mast 2036** (was 115R / 114R) stay **CO-3-dwarf** (Slow Clear / Restricting / Stop — Restricting is yellow). All other Digicon **LAMP1** masts use `CO-3-dwarf` on Layout Editor (T6, OS S-R, OS 31, OS W-1/OS W-2, OS K-1/OS K-2). Packed IDs: Mast 2036 `IH1133` / Mast 2035 `IH1134`; east Princess dwarfs `IH1136` (40LA) on C11; west Princess `IH143` (38LA) on C1.

- Packing: `displayNode*100 + UID` (`UID = 32 + signal_index`) — see `mqtt_serial.h`
- Appearances: stock C&O-1980 `CO-33-hi` two-head / `CO-3-dwarf` (not `cats-masts` / not `hart-aar`)
- Digicon `PHYSIGNAL`: templates remapped to C&O-1980 names (`cats/scripts/aar_aspect_bridge.py`); CATS CTC is `HOLD_ONLY`; CATS ABS SECSIGNAL names are unbound (`unbind_abs_from_jmri_masts.py`) so SML owns LE
