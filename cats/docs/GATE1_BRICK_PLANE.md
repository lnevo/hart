# Gate 1 — Brick → Block 100-102 → Plane
...
| C (continuing) | east | Block 100-102 | yes |

Then: Block 100-102 → TO102 A (Plane) → East Main Ext (B).
...
**Critical:** Digicon must put Block 100-102 on the **continuing HORIZONTAL**, not on a diverge slash into the next plant throat.
...
| 2 | SWITCHPOINTS (LH) | OS 100 (Brick) | Block 4-2 | Continuing = east straight |
| 3 | yard diverge | OS 101 (Brick) | Block 4-1 | Off the main spine |
| 4 | HORIZONTAL (long) | Block 100-102 | Block 4-6 | Mid-spine; not plant approach |
| 5 | SWITCHPOINTS | OS 102 (Plane) | Block 4-5 | Plane diverge |
...
- [ ] `M2S405` ACTIVE → red on **HORIZONTAL** Block 100-102 only
...
Until Designer save exists, generator `--only gate1` clones the **contiguous** Armstrong Brick→Plane window (`X3–20`) and maps Intermediate1 → Block 100-102 (HORIZONTAL between plants). Do **not** abut separate bands (ClassCast VitalLogic). Interim is still not Neville LH100 geography — **Designer Gate 1 replaces it**.